...
function main()
    ...
    input_topology = (2, 2)
    input_warmup = 5
    input_runs = 1  # Number of times to run the simulation
    ...
    # Collect walltimes from multiple runs
    run_walltimes = Float64[]
    
    # Initialize MPI once if multiple runs and not yet initialized
    owns_mpi_global = false
    if input_runs > 1 && !MPI.Initialized()
        MPI.Init()
        owns_mpi_global = true
    end
    
    for run_id in 1:input_runs
        # Skip finalization between runs, only finalize at the very end
        should_skip_finalize = input_runs > 1
        walltime = run_baseline(input_nx, input_ny; nt=input_nt,
            outdir = input_outdir,
            do_viz = input_do_viz && !input_benchmark,
            benchmark = input_benchmark,
            topology = input_topology,
            warmup = input_warmup,
            skip_mpi_finalize = should_skip_finalize
        )
        push!(run_walltimes, walltime)
    end

    # Save all run walltimes to CSV if more than one run (rank 0 only)
    if input_runs > 1 && MPI.Comm_rank(MPI.COMM_WORLD) == 0
        
        topo_str = "$(input_topology[1])x$(input_topology[2])"
        mkpath(input_outdir)
        fname = joinpath(input_outdir, "run_walltimes_$(topo_str).csv")
        df = DataFrame(run_id=1:input_runs, walltime_seconds=run_walltimes)
        CSV.write(fname, df)
        @info "Saved run walltimes to $fname"
        
        # Finalize MPI if we initialized it for multiple runs
        if owns_mpi_global
            MPI.Finalize()
        end
    end
